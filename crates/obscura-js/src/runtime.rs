...
impl ObscuraJsRuntime {
    ...
    pub fn set_intercept_page_id(&self, page_id: String) {
        self.state.borrow_mut().intercept_page_id = Some(page_id);
    }
...
mod tests {
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn service_worker_interfaces_are_defined_globally() {
        let mut rt = setup_runtime("<div></div>");
        let result = rt
            .evaluate(
                r#"
