...
fn accept_dispatch(
    ...
    let identity = match parse_upgrade_context(head.as_bytes(), stealth) {
...
fn interception_route(
    ...
    let page_id = intercepted.page_id.as_ref()?;
    let session_id = ctx.fetch_intercept.sessions_by_page.get(page_id)?.clone();
    if let Some(session) = session_id.as_ref() {
        if ctx.sessions.get(session) != Some(page_id) {
            return None;
        }
    }
    let page = ctx.pages.iter().find(|page| &page.id == page_id)?;
    Some((session_id, page.frame_id.clone()))
}
...
fn emit_intercepted_request(
    ...
    if !intercepted.body.is_empty() {
        ...
        request["postData"] = json!(String::from_utf8_lossy(&intercepted.body));
        request["postDataEntries"] = json!([{
            "bytes": base64::engine::general_purpose::STANDARD.encode(&intercepted.body),
        }]);
    }
...
fn sync_live_page_network_events(ctx: &mut CdpContext) {
    ...
    let live_ids: Vec<String> = ctx
        ...
        .collect();
    for page_id in live_ids {
        let session_ids: Vec<Option<String>> = ctx.sessions.iter()
...
async fn process_with_interception(
    ...
    let interception_session = ctx.fetch_intercept.sessions_by_page.get(&page_id)
        .cloned().unwrap_or_else(|| req.session_id.clone());
...
mod tests {
    ...
    async fn interception_routes_to_the_enabling_session_on_the_owning_page() {
        let mut ctx = crate::dispatch::CdpContext::new();
        let first = ctx.create_page();
        let second = ctx.create_page();
        ctx.sessions.insert("first-managed".into(), first);
        ctx.sessions.insert("second-managed".into(), second.clone());
        ctx.sessions.insert("second-raw".into(), second.clone());
        crate::domains::fetch::handle(
            "enable", &json!({}), &mut ctx, &Some("second-raw".into()),
        ).await.unwrap();
        let (resolver, _receiver) = tokio::sync::oneshot::channel();
        let request = obscura_js::ops::InterceptedRequest {
            page_id: Some(second.clone()),
            body: b"cursor=next".to_vec(),
            request_id: "second-intercept-1".into(),
        ...
        let expected_frame = ctx.get_page_mut(&second).unwrap().frame_id.clone();
        assert_eq!(super::interception_route(&ctx, &request),
            Some((Some("second-raw".into()), expected_frame)));
        ctx.sessions.remove("second-raw");
        assert!(super::interception_route(&ctx, &request).is_none(),
            "a detached subscriber must not redirect its request to another session");
        ctx.remove_page(&second);
        assert!(!ctx.fetch_intercept.sessions_by_page.contains_key(&second));
    }

    #[test]
    fn fragmented_identity_header_waits_for_complete_head() {
        use std::io::Write;
        use std::net::{TcpListener, TcpStream};
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let mut client = TcpStream::connect(listener.local_addr().unwrap()).unwrap();
        let (stream, _) = listener.accept().unwrap();
        client.write_all(b"GET /devtools/browser HTTP/1.1\r\nX-Obscura-Con").unwrap();
        assert!(matches!(super::peek_request_head(&stream), super::PeekStatus::NotReady));
        client.write_all(b"text: ignored\r\n\r\n").unwrap();
        // TCP can deliver the second write on a later accept-loop sweep.
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(1);
        loop {
            match super::peek_request_head(&stream) {
                super::PeekStatus::Head(head) => {
                    assert!(head.contains("X-Obscura-Context: ignored\r\n\r\n"));
                    break;
                }
                super::PeekStatus::NotReady => {
                    assert!(std::time::Instant::now() < deadline, "second fragment did not arrive");
                    std::thread::sleep(std::time::Duration::from_millis(1));
                }
                _ => panic!("fragmented handshake was rejected"),
            }
        }
    }
    ...
    async fn js_network_events_reach_every_session_attached_to_the_page() {
        ...
        for probe in 0..2 {
            ...
            expected.extend([managed, flattened]);
        }
