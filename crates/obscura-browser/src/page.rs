...
impl Page {
    ...
    async fn run_next_pending_frame(&mut self) -> bool {
        ...
        while let Some((frame_id, url)) = self.pending_frame_work.front().and_then(|pending| {
            ...
            let source = if self.should_block_url(&url) {
                ...
                decode_data_uri(&url).map(|body| String::from_utf8_lossy(&body).into_owned())
    ...
    fn init_js(&mut self) {
        ...
        rt.set_intercept_page_id(self.id.clone());
    ...
    pub fn sync_js_network_events(&mut self) {
        ...
        for ev in events {
            self.network_events.push(NetworkEvent {
                ...
                post_data_bytes: ev.post_data_bytes,
                ...
                match listener.accept() {
                    Ok((mut stream, _)) => {
                        ...
                        std::thread::spawn(move || {
                            ...
                            // Large font writes must not truncate on WouldBlock.
                            stream.set_nonblocking(false).unwrap();
                            stream.set_write_timeout(Some(std::time::Duration::from_secs(2))).unwrap();
                            // TCP reads may split the request headers. Closing with
                            // unread bytes can reset the connection and truncate the
                            // response, making resource tests depend on packet timing.
